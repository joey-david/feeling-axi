...
SMALL_MODELS = [
    ...
    ("google/gemma-2-9b",                  "Gemma_2_9B_base",       "raw",  12, 21),
    ("google/gemma-2-9b-it",               "Gemma_2_9B_instruct",   "chat", 12, 12),
    ("meta-llama/Llama-3.1-8B",            "Llama_3.1_8B_base",     "raw",  12, 16),
    ("meta-llama/Llama-3.1-8B-Instruct",   "Llama_3.1_8B_instruct", "chat", 12, 16),
    ("mistralai/Mistral-7B-v0.1",          "Mistral_7B_base",       "raw",  12, 16),
    ("mistralai/Mistral-7B-Instruct-v0.1", "Mistral_7B_instruct",   "chat", 12, 16),
    ("microsoft/phi-4",                    "Phi_4",                 "chat",  6, 12),
    ("Qwen/Qwen2.5-7B",                    "Qwen_2.5_7B_base",      "raw",   8, 16),
    ("Qwen/Qwen2.5-7B-Instruct",           "Qwen_2.5_7B_instruct",  "chat",  8, 16),
    ("Qwen/Qwen3-8B",                      "Qwen_3_8B_base",        "raw",  21, 27),
    ("Qwen/Qwen3-14B",                     "Qwen_3_14B_base",       "raw",  16, 20),
]
    ...
    with torch.no_grad():
        for name, W, scale in residual_write_matrices(model):
            ...
            if name == "embed":
                ...
                s = torch.where(scale.abs() < 1e-4,
                                torch.full_like(scale, 1e-4), scale)
                SW = W32 * s.unsqueeze(1)
                corr = r.T @ (r @ SW)
                W32 -= corr / s.unsqueeze(1)
...
print("\nSmall models:")
for i, (repo, name, fmt, l1, l2) in enumerate(SMALL_MODELS, start=1):
...
SELECTED = SMALL_MODELS if choice == "all" else [SMALL_MODELS[int(choice) - 1]]
